...
function iframeLoad(sign) {
	$('#' + sign + '_upload_return').load(function () {
		...
		if(data.indexOf('413')>0){
			var error = $('<div><label id="' + sign + 'Text-error" class="error" style="display: inline;" for="' + sign + 'Text">' + "请上传2M以内的jpg、gif、bmp、png、jpeg格式图片" + '</label></div>');
...
function createUploadForm(target) {
	//alert("测试上传");
	//alert(getRootPath());
	var domain=fts.baseURI;
	//if(''==domain||null==domain){
		//domain="http://wx.hongbaoxiche.cn"
	//}
	//alert(domain);
	var uploadForm = $('<form id="uploadForm" method="post" action="' + domain + '/img/upload.act" target="' + target + '" enctype="multipart/form-data" style="display:none"></form>');
